...
<div class="table-responsive col-lg">
  <a href="/dashboard/categories/create" class="btn btn-primary mb-3">Tambah Kategori</a>
        <table class="table table-striped table-sm" id="kategoriberita">
          <thead>
            <tr>
              <th scope="col">#</th>
        ...
        <tr>
          ...
          <td>{{ $category->name }}</td>
          <td>
                <a href="/dashboard/category/edit/{{ $category->slug }}" class="badge bg-warning"><span data-feather="edit"></span></a>
                <form action="/dashboard/kategoriberita/hapus/{{ $category->slug }}" method="post" class="d-inline">
                  @method('delete')
                  @csrf
